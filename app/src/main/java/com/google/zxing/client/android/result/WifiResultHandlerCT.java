 ...
 * @author Sean Owen
 */
public final class WifiResultHandlerCT extends ResultHandler {

  private static final String TAG = WifiResultHandlerCT.class.getSimpleName();

  private final CaptureActivityCT parent;

  public WifiResultHandlerCT(CaptureActivityCT activity, ParsedResult result) {
  ...
  public void handleButtonPress(int index) {
    if (index == 0) {
      ...
      activity.runOnUiThread(new Runnable() {
        @Override
        public void run() {
          Toast.makeText(activity.getApplicationContext(), com.google.zxing.client.android.R.string.wifi_changing_network, Toast.LENGTH_SHORT).show();
        }
      });
