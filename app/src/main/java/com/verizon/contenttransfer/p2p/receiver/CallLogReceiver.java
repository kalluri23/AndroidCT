...
public class CallLogReceiver {
	private static final String TAG = CallLogReceiver.class.getName();

	public static void receiveCalllogs(Socket iosServerSocket) {
		String mediaType =  "";
		try {
			...
			clientOutputStream.write(VZTransferConstants.CALLLOG_REQUEST_HEADER.getBytes());
			...
			String FILE_NAME = VZTransferConstants.CLIENT_CALLLOG_LIST_FILE;		//"Calllogs";
			...
			try {
				...
				int readSize = 0;
				DataSpeedAnalyzer.setProgressbarProperties(CTGlobal.getInstance().getContentTransferContext().getString(R.string.callLogs_str), VZTransferConstants.CLIENT_CALLLOG_LIST_FILE);
				if ( imageSize > 0 ) {
