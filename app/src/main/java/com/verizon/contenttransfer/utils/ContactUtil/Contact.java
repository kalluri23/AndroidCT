...
import com.verizon.contenttransfer.utils.Base64Coder;
import com.verizon.contenttransfer.utils.LogUtil;
import com.verizon.contenttransfer.utils.QuotedPrintable;
import com.verizon.contenttransfer.utils.StringUtil;
...
public class Contact {
    	...
    	while (line != null) {
    		...
    		if (pm.matches()) {
    			...
    			if (prop.equalsIgnoreCase("END") && val.equalsIgnoreCase("VCARD")) {
    				// End of vCard
					//LogUtil.e(TAG, "returning parseLen " + parseLen);
    				return parseLen;
    			}
    			...
    			if (!ppm.find()) {
					...
					//LogUtil.e(TAG, "not a valid property");
					continue;
				}
    			...
    			if (encoding.equalsIgnoreCase("QUOTED-PRINTABLE")) {
    				try {
						...
						|| encoding.equalsIgnoreCase("b")) { // Android send ENCODING=BASE64, iOS send ENCODING=b
    				...
    				do {
    					...
    					if (line == null){
							...
							//LogUtil.e(TAG, "line length is 0");
							break;
						}

						Matcher pm1 = propPattern.matcher(line);

						if (pm1.matches()) {
							String prop1 = pm1.group(1);
							String val1 = pm1.group(2);

							if (prop1.equalsIgnoreCase("END") && val1.equalsIgnoreCase("VCARD")) {
								skipRead = true;
								//LogUtil.e(TAG, "line was END:VCARD"");
								break;
							}
						}
						else if (!base64Pattern.matcher(line).matches()){
							//LogUtil.e(TAG, "Not a base64 pattern");
							break;
						}
   						tmpVal.append(line);
    				} while (true);
     ...
     * @param im The rowdata containing the actual phone data.
     */
     ...
     * @param org The rowdata containing the actual organization data.
     */
     ...
     */
    public void addContact(Context context, long key, boolean replace) {
		...
		for(Contact.RowData phone : phones) {
			if (phone.data != null) {
		...
		for(Contact.RowData emailID : emails) {
			//------------------------------------------------------ Email
			if (emailID != null) {
				ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
						.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0)
						.withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.Email.CONTENT_ITEM_TYPE)
						.withValue(ContactsContract.CommonDataKinds.Email.DATA, emailID.data)
						.withValue(ContactsContract.CommonDataKinds.Email.TYPE, emailID.type)
						.build());
			}
		}
		for(Contact.RowData addr : addrs) {
			//------------------------------------------------------ Address
			if (addr != null)
				ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
						.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0)
						.withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.StructuredPostal.CONTENT_ITEM_TYPE)
						.withValue(ContactsContract.CommonDataKinds.StructuredPostal.DATA, addr.data)
						.withValue(ContactsContract.CommonDataKinds.StructuredPostal.TYPE, addr.type)
						.build());
		}

		for(Contact.RowData im : ims) {
			if(im != null)
				ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
						.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0)
						.withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.Im.CONTENT_ITEM_TYPE)
						.withValue(ContactsContract.CommonDataKinds.StructuredPostal.DATA, im.data)
						.withValue(ContactsContract.CommonDataKinds.StructuredPostal.TYPE, im.type)
						.build());
		}
		//------------------------------------------------------ Organization
		for(Contact.OrgData org : orgs) {
			if (org.company != null && org.title != null) {
				ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
						.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0)
						.withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.Organization.CONTENT_ITEM_TYPE)
						.withValue(ContactsContract.CommonDataKinds.Organization.COMPANY, org.company)
						.withValue(ContactsContract.CommonDataKinds.Organization.TYPE, org.type)
		...
		try {
			LogUtil.d(TAG, "Trying to add contact: " + displayName);
    ...
    /*
        ...
        if (key <= 0 && syncid != null) {
			//LogUtil.d(TAG, "***key is 0 and syncID is null");
        ...
        if (key > 0) {
			//LogUtil.d(TAG, "***key is higher than 0");
        ...
        if (newContactUri == null) {
        	...
        	if (newContactUri == null) {
            	LogUtil.e(TAG, "Error adding contact." + " (key: " + key + ")");
            	return -1;
            }
			...
			//Contacts.People.addToGroup(cResolver, key, 1);

        } else {
        	// update existing Uri
    		if (!replace) {
				//LogUtil.e(TAG, "Not replacing existing contact");
				return -1;
			}
    		replacing = true;
	...
	public long vcardCount(BufferedReader vCard) throws IOException {
		// Reset the currently read values.
		reset();
